...
namespace WebHost.Utils.MemoryManagement;
...
public sealed class PooledMemoryOwner : IMemoryOwner<byte>
{
    ...
    public PooledMemoryOwner(byte[] buffer, int length, ArrayPool<byte> pool)
    {
        ...
        // Wrap the buffer in a Memory<byte> for safe, sliceable memory access
    ...
    /// This provides access to the buffer in a safe, sliceable, and memory-efficient way.
    /// </summary>
