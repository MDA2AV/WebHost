...
public sealed partial class WebHostApp<TContext>
{
    ...
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous operation. When complete, the <see cref="WebHostApp{TContext}"/> instance is fully started.</returns>
    public async Task<WebHostApp<TContext>> StartAsync()
    {
    ...
    /// </summary>
    /// <returns>The current <see cref="WebHostApp{TContext}"/> instance for method chaining or further interaction.</returns>
    public WebHostApp<TContext> Start()
    {
        InternalHost.Start();
        return this;
    }
